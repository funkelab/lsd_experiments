...
def train_until(max_iteration):
    ...
    last = affinity_neighborhood[9:12]
    ln = [last[0][0],last[1][1],last[2][2]]

    labels_padding = Coordinate(
                        [round(
                            int(
                                np.sqrt(2)*(output_size[d]/2 + np.abs(ln[d]))
                                )
                            )
                            for d in range(output_size.dims())
                        ]
                    )

    labels_padding = Roi(labels_padding,(0,)*3).snap_to_grid(
                        voxel_size,mode='shrink').get_begin()
