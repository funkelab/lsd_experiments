...
def train_until(max_iteration):
    ...
    auto_input_size = Coordinate(auto_config['input_shape'])*voxel_size
    context_input_size = Coordinate(context_config['input_shape'])*voxel_size
    pretrained_affs_size = Coordinate(context_config['input_shape'])*voxel_size
    output_size = Coordinate(context_config['output_shape'])*voxel_size
    context = output_size/2

    request = BatchRequest()
    request.add(raw, auto_input_size)
    ...
    train_pipeline = (
        ...
        Predict(
            ...
            outputs={
                auto_config['affs']: pretrained_affs
            }) +
        EnsureUInt8(pretrained_affs) +
        ...
        Train(
            ...
            inputs={
                ...
                context_config['pretrained_affs']: pretrained_affs,
                context_config['gt_affs']: gt_affs,
                context_config['loss_weights_affs']: gt_affs_scale,
            },
            outputs={
                context_config['auto_affs']: affs
            },
            gradients={
                context_config['auto_affs']: affs_gradient
            },
        ...
        Snapshot({
                raw: 'volumes/raw',
                labels: 'volumes/labels/neuron_ids',
                pretrained_affs: 'volumes/labels/pretrained_affs',
