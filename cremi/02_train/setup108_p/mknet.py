...
def create_auto(input_shape, output_shape, name):
    ...
    with tf.variable_scope('setup100_p'):
        ...
        affs_batched = crop_zyx(affs_batched, (1, 12) + output_shape)
        affs = tf.reshape(affs_batched, (12,) + output_shape)
...
def create_affs(input_shape, intermediate_shape, expected_output_shape, name):
    ...
    with tf.variable_scope('setup108_p'):
        ...
        pretrained_affs = tf.placeholder(tf.float32, shape=(12,) + intermediate_shape)
        pretrained_affs_batched = tf.reshape(pretrained_affs, (1, 12) + intermediate_shape)
        ...
        affs_loss_weights = tf.placeholder(tf.float32, shape=(12,) + output_shape)
        ...
        summary = tf.summary.scalar('setup108_eucl_loss', loss)
...
if __name__ == "__main__":
    ...
    create_config(train_input_shape, train_output_shape, 12, 'config')
