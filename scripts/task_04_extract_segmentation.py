...
class SegmentationTask(daisy.Task):
    ...
    def prepare(self):
        ...
        print("Reading graph from DB ", self.db_name, self.edges_collection)
        start = time.time()
        ...
        if 'id' not in node_attrs:
            print('No nodes found in roi %s' % self.roi)
            return
        ...
        print("Complete RAG contains %d nodes, %d edges" % (len(self.nodes), len(self.edges)))
