...
def evaluate(
    ...
    scores_config = {
            ...
            'setup': setup,
            'iteration': iteration,
            'network_configuration': edges_db_name + '_multicut',
            'merge_function': 'multicut'
            }
    ...
    fragment_segment_lut_file = os.path.join(
        ...
        'luts',
        'fragment_segment',
        'lut_vanilla_mc_s3_%s.npz'%setup)
    ...
    number_of_segments = len(stats['merge_stats'].keys())
    number_of_skeletons = len(stats['split_stats'].keys())
    ...
    print("Calculating scores for multicut...")
