...
class EvaluateAnnotations():
    ...
    def __init__(
        ...
        self.site_fragment_lut_directory = os.path.join(
            self.fragments_file,
            'luts/site_fragment/new_gt_9_9_20/testing')
    ...
    def read_skeletons(self):
        ...
        if self.run_type:
            ...
            # node_mask = ''.join([node_mask, '_', self.run_type.strip('_masked')])
            # node_components = ''.join([node_components, '_', self.run_type.strip('_not_masked')])
            node_mask = node_mask + '_' + self.run_type.strip('_ffn')
            node_components = node_components + '_' + self.run_type.strip('_ffn')
