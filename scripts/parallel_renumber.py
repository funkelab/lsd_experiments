...
def renumber_in_block(block, segmentation_ds, id_map, ignore=[0]):
    ...
    if len(old_ids > 0):
        ...
        new_ids = np.array([id_map[old_id] for old_id in old_ids], dtype=old_ids.dtype)
        ...
        logging.debug("Block {0} contains no relevant IDs".format(block.block_id))
...
def parallel_renumber(
    ...
    id_map = {old_id: i+1 for i, old_id in enumerate(id_registry)}
    print(len(id_map))
