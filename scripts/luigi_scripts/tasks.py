...
class ExtractAgglomerate(luigi.Task):
    ...
    def run(self):
        ...
        output_base = os.path.join(self.output_dir(), self.sample) + '_extract_agglomerate'
        log_out = output_base + '.out'
        log_err = output_base + '.err'
        ...
        with open(log_out, 'w') as o:
            with open(log_err, 'w') as e:
                check_call([
                    ...
                    'python -u extract_agglomerate_blockwise.py ' + config_filename
                ], stdout=o, stderr=e)
