...
def create_network(input_shape, name):
    ...
    with tf.variable_scope('setup15'):

        raw = tf.placeholder(tf.float32, shape=input_shape)
        raw_batched = tf.reshape(raw, (1, 1) + input_shape)

        print('Raw, Raw_batched shapes')
        print(raw, raw_batched)
        ...
        loss_weights_embedding = tf.reshape(loss_weights, (1,) + tuple(embedding_output_shape[1:]))
        ...
        summary = tf.summary.merge([
            tf.summary.scalar('setup15_eucl_loss', loss),
            tf.summary.scalar('setup15_eucl_loss_embedding', loss_embedding),
            tf.summary.scalar('setup15_eucl_loss_affs', loss_affs)
        ])
