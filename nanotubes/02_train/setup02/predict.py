...
class ThresholdMask(BatchFilter):
    ...
    def process(self, batch, request):
        batch[self.array].data = batch[self.array].data >= self.threshold
...
def predict(
    ...
    raw = ArrayKey('RAW')
    pred_labels = ArrayKey('PRED_LABELS')

    chunk_request = BatchRequest()
    chunk_request.add(raw, input_size)
    chunk_request.add(pred_labels, output_size)
