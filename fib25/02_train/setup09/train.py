...
def add_malis_loss(graph):
    ...
    gt_affs_mask = tf.placeholder(tf.int64, shape=(3, 56, 56, 56), name='gt_affs_mask')
    ...
    lsd_loss = graph.get_tensor_by_name(config['loss_lsds'])
    loss = malis_loss + lsd_loss 

    optimizer = opt.minimize(loss)

    malis_summary = tf.summary.scalar('setup09_malis_loss', malis_loss)
    lsd_summary = tf.summary.scalar('setup09_lsd_loss', lsd_loss)
    comb_summary = tf.summary.scalar('setup09_comb_loss', loss)

    summary = tf.summary.merge([
        malis_summary,
        lsd_summary,
        comb_summary],
        name='setup09_merged_loss')
...
def train_until(max_iteration):
    ...
    # train_pipeline += UnmaskBackground(gt_lsds_scale, labels_mask)
    ...
    if phase == 'euclid':
        ...
        train_summary = 'setup09_merged_loss/setup09_merged_loss:0'
